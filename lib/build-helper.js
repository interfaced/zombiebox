...
const logger = createChild('Build');
...
class BuildHelper {
	...
	getCompressedScripts(flags = {}) {
		...
		const closureCompiler = new GoogleClosureCompiler.compiler(compilerOptions);
	...
	async getCompressedStyles(resourcesTargetPath) {
		const allStyles = await this._application.getSortedStyles();
