...
class SourceProviderFS extends SourceProviderBase {
	 ...
	 */
	watch() {
		logger.verbose(`Watching ${chalk.underline(this._root)} for filesystem events`);
		if (this._watcher) {
	...
	_onWatcherEvent(event, filePath) {
		...
		switch (event) {
			case 'change':
				...
				logger.silly(`Watched file changed: ${chalk.underline(filePath)}`);
			...
			case 'unlink':
				...
				logger.silly(`Watched file removed: ${chalk.underline(filePath)}`);
			...
			case 'add':
				...
				logger.silly(`File added: ${chalk.underline(filePath)}`);
