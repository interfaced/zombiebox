...
import {mergeConfigs, loadJSON} from '../utils.js';
import {Validator} from 'jsonschema';
import {IZombieBoxConfig} from './interface.js';
import {createChild} from '../logger.js';
const logger = createChild('Config');
const schema = await loadJSON(import.meta.url, 'schema.json');
...
class Config {
	...
	async loadFile(src) {
		...
		const configFn = (await import(src)).default;
		this.appendObject(configFn(this._config));
	}
