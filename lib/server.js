...
class Server {
	 ...
	 */
	rawProxy(route) {
		logger.info(`Proxy enabled at ${chalk.cyan(route + '/?url=...')}`);
	 ...
	 */
	proxy(route, address) {
		logger.info(`Proxying path ${chalk.green(route)} from ${chalk.cyan(address)}`);
		this.use(route, (req, res) => {
	...
	_preprocessModule(content, fsPath) {
		...
		try {
			...
			logger.error(`Error while resolving module paths in ${chalk.underline(fsPath)}: ${e.toString()}`);
		...
		for (const node of ast.body.reverse()) {
			...
			if (node.type === 'ImportDeclaration') {
				...
				if (source.type === 'Literal' && !PathHelper.isLocal(source.value)) {
					...
					// logger.silly(
					// 	`Replacing import path ${chalk.red(source.value)} ` +
					// 	`with ${chalk.green(webPath)} in ` +
					// 	chalk.underline(fsPath)
					// );
